...
result_tag = '_OAA_{:.1}.jpg'.format(oaa_threshold)
...
def attention(images):
    ...
    for image in images[1:]:
        at_image = np.maximum(at_image, image)
    return at_image
...
for image_path, label in train_dic['daisy']:
    ...
    oaa_npy_name = image_path.replace(flags.root_dir, '').replace('.jpg', '_daisy.png.npy') # history
    oaa_npy_path = oaa_dir + oaa_npy_name
    ...
    data = list(np.load(oaa_npy_path, allow_pickle = True))

    attention_maps = np.asarray([image for image, _ in data], dtype = np.uint8)
    thresholds = np.asarray([th for _, th in data], dtype = np.float32)

    attention_maps = attention_maps[thresholds >= oaa_threshold]
    print(pred, label, len(data), len(attention_maps))
